...
TransitInStrategy::TransitInStrategy(
  rclcpp::Node & node,
...
void TransitInStrategy::on_enter()
{
  waypoint_index_ = 0;
  ...
  RCLCPP_INFO(
    node_.get_logger(),
    "[TRANSIT_IN] Transit In activated with %zu waypoint(s), altitude %.2f m above home, arrival radius %.2f m, horizontal speed %.2f m/s",
    waypoints_.size(), transit_altitude_above_home_m_, arrival_radius_m_, max_horizontal_speed_m_s_);
}

void TransitInStrategy::on_exit()
{
  RCLCPP_INFO(node_.get_logger(), "[TRANSIT_IN] Transit In deactivated");
}

void TransitInStrategy::on_update(float dt_s)
{
  (void)dt_s;
  ...
  if (!snapshot.is_armed) {
    fail("vehicle is not armed; Transit In requires an airborne vehicle");
    return;
  }

  // Check land detection freshness and landed state
  if (!snapshot.land_detected_valid) {
    if (data_timed_out()) {
      fail("no fresh vehicle_land_detected sample was received after activation");
    } else {
      RCLCPP_INFO_THROTTLE(
        node_.get_logger(), *node_.get_clock(), 1000,
        "[TRANSIT_IN] Waiting for a fresh PX4 vehicle_land_detected sample before starting Transit In");
    }
    return;
  }

  if (snapshot.is_landed) {
    fail("vehicle is landed; Transit In must be selected while airborne");
    return;
  }

  // Check home position validity (latched state in PX4)
  if (!snapshot.home_pos_valid) {
    if (data_timed_out()) {
      ...
      RCLCPP_INFO_THROTTLE(
        node_.get_logger(), *node_.get_clock(), 1000,
        "[TRANSIT_IN] Waiting for a valid PX4 home position before starting Transit In");
    }
    return;
  }
  home_altitude_msl_m_ = snapshot.home_global_position.z();

  // Check global and local position and vertical velocity
  if (!snapshot.global_pos_valid || !snapshot.local_pos_valid) {
    if (data_timed_out()) {
      fail("required PX4 position data became invalid during Transit In");
    } else {
      RCLCPP_INFO_THROTTLE(
        node_.get_logger(), *node_.get_clock(), 1000,
        "[TRANSIT_IN] Waiting for valid PX4 global and local position data before starting Transit In");
    }
    return;
  }
  ...
  if (!target_altitude_set_) {
    ...
    RCLCPP_INFO(
      node_.get_logger(),
      "[TRANSIT_IN] Transit In target altitude is %.2f m AMSL", target_altitude_msl_m_);
  }
  ...
  if (waypoint_index_ >= waypoints_.size()) {
    mode_finished_ = true;
    RCLCPP_INFO(node_.get_logger(), "[TRANSIT_IN] Transit In completed all %zu waypoints successfully", waypoints_.size());
    if (completion_cb_) {
      completion_cb_(true);
    }
    return;
  ...
  const domain::RoutePoint & wp = waypoints_[waypoint_index_];
  double target_alt = target_altitude_msl_m_;
  if (wp.altitude_m > 0.0 && wp.altitude_m != 10.0 && wp.altitude_m != 20.0 && wp.altitude_m != transit_altitude_above_home_m_) {
  ...
  RCLCPP_INFO(
    node_.get_logger(),
    "[TRANSIT_IN] Transit In reached waypoint %zu/%zu (lat=%.6f, lon=%.6f, alt=%.2f m)",
    waypoint_index_ + 1U, waypoints_.size(), wp.latitude_deg, wp.longitude_deg, target_alt);

  // Persist durable route checkpoint if persistence manager is configured
  if (persistence_) {
    persistence::JournalEntry entry;
    entry.event_id = "EVT_TRANSIT_IN_WAYPOINT_REACHED";
    entry.component = "TransitInStrategy";
    entry.detail = "Reached waypoint " + std::to_string(waypoint_index_ + 1U) + "/" + std::to_string(waypoints_.size());
...
std::optional<float> TransitInStrategy::update_course_heading(const adapters::Px4StateSnapshot & snapshot)
{
  if (snapshot.local_pos_valid) {
    ...
    if (std::isfinite(horizontal_speed) &&
        horizontal_speed >= course_heading_min_speed_m_s_ &&
...
bool TransitInStrategy::waypoint_reached(
  ...
  const Eigen::Vector3d & target) const
{
  ...
  if (!std::isfinite(horizontal_distance_m) ||
      horizontal_distance_m > arrival_radius_m_ ||
      !std::isfinite(altitude_error_m) ||
      altitude_error_m > altitude_tolerance_m_) {
    return false;
  }
...
bool TransitInStrategy::data_timed_out() const
{
  if (state_cache_) {
...
void TransitInStrategy::fail(const std::string & reason)
{
  if (mode_finished_ || failed_) {
    return;
  }
  ...
  RCLCPP_ERROR(node_.get_logger(), "[TRANSIT_IN] Transit In failed: %s", reason.c_str());
  if (completion_cb_) {
