...
namespace
{
constexpr char kModeName[] = "Transit In";
constexpr char kNodeName[] = "transit_in";
constexpr bool kEnableDebugOutput = true;
}

TransitIn::TransitIn(rclcpp::Node & node)
: ModeBase(node, px4_ros2::ModeBase::Settings{kModeName, false}), _node(node)
{
  ...
  _home_position_sub = _node.create_subscription<px4_msgs::msg::HomePosition>(
    ...
    std::bind(&TransitIn::homePositionCallback, this, std::placeholders::_1));
  ...
  _vehicle_land_detected_sub = _node.create_subscription<px4_msgs::msg::VehicleLandDetected>(
    ...
    std::bind(&TransitIn::vehicleLandDetectedCallback, this, std::placeholders::_1));

  loadParameters();
}

void TransitIn::loadParameters()
{
  _node.declare_parameter<double>("transit_in_alt", 10.0);
  ...
  _node.get_parameter("transit_in_alt", _transit_altitude_above_home_m);
  ...
  if (!std::isfinite(_transit_altitude_above_home_m) ||
    ...
    RCLCPP_ERROR(_node.get_logger(), "transit_in_alt must be finite and non-negative");
    valid = false;
  } else if (_transit_altitude_above_home_m > kHardMaxAltitudeAboveHomeM) {
    RCLCPP_WARN(
      _node.get_logger(),
      "transit_in_alt %.2f exceeds the hard altitude cap %.2f; clamping",
...
void TransitIn::onActivate()
{
  ...
  RCLCPP_INFO(_node.get_logger(), "Transit In activated");
}

void TransitIn::onDeactivate()
{
  RCLCPP_INFO(_node.get_logger(), "Transit In deactivated");
}

void TransitIn::updateSetpoint(float)
{
  if (_mode_finished) {
    return;
  }
  ...
  if (!isArmed()) {
    failMode("vehicle is not armed; Transit In requires an airborne vehicle");
    return;
  }
  ...
  if (_waiting_for_fresh_land_state || !_land_state_valid) {
    if (dataTimedOut()) {
      ...
      RCLCPP_INFO_THROTTLE(
        ...
        "Waiting for a fresh PX4 vehicle_land_detected sample before starting Transit In");
    }
    return;
  }
  ...
  if (land_state_age.count() > _data_timeout_s) {
    failMode("vehicle_land_detected data became stale during Transit In");
    return;
  }

  if (_landed) {
    failMode("vehicle is landed; Transit In must be selected while airborne");
    return;
  }
  ...
  if (!_home_position_valid) {
    if (dataTimedOut()) {
      ...
      RCLCPP_INFO_THROTTLE(
        _node.get_logger(), *_node.get_clock(), 1000,
        "Waiting for a valid PX4 home position before starting Transit In");
    }
    return;
  }
  ...
  if (!_vehicle_global_position->positionValid() ||
    ...
    if (_setpoint_sent_for_current_waypoint || dataTimedOut()) {
      failMode("required PX4 position data became invalid during Transit In");
    } else {
      RCLCPP_INFO_THROTTLE(
        _node.get_logger(), *_node.get_clock(), 1000,
        "Waiting for valid PX4 global and local position data before starting Transit In");
    }
    return;
  }
  ...
  if (!_target_altitude_set) {
    ...
    RCLCPP_INFO(
      _node.get_logger(), "Transit In target altitude is %.2f m AMSL", _target_altitude_msl_m);
  }
  ...
  RCLCPP_INFO(
    _node.get_logger(), "Transit In reached waypoint %zu/%zu", _waypoint_index + 1U,
    _waypoints.size());
  ++_waypoint_index;
  _setpoint_sent_for_current_waypoint = false;
...
void TransitIn::homePositionCallback(const px4_msgs::msg::HomePosition::SharedPtr msg)
{
...
void TransitIn::vehicleLandDetectedCallback(
...
std::optional<float> TransitIn::updateCourseHeading()
{
...
bool TransitIn::waypointReached(const Eigen::Vector3d & target) const
{
...
bool TransitIn::dataTimedOut() const
{
...
void TransitIn::failMode(const std::string & reason)
{
  if (_mode_finished) {
    return;
  }
  ...
  _mode_finished = true;
  RCLCPP_ERROR(_node.get_logger(), "Transit In failed: %s", reason.c_str());
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<px4_ros2::NodeWithMode<TransitIn>>(
