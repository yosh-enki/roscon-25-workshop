...
docker run --rm -it \
    ...
    bash -c 'source /opt/ros/humble/setup.bash && source /home/ubuntu/px4_ros_ws/install/setup.bash && source install/setup.bash && (python3 full_self_driving/scripts/esp32_gripper_bridge.py --ros-args -p port:="'"${SERIAL_PORT}"'" &) && ros2 launch full_self_driving full_self_driving.launch.py simulation:=true world:="'"${WORLD}"'" engineering_config:=/home/ubuntu/roscon-25-workshop/full_self_driving/config/fsd_parameters.yaml headless:=false payload_adapter:=px4_uorb_gripper_actuator foxglove_port:="'"${FOXGLOVE_PORT}"'"'
